...
class PeopleOpsHandler(SimpleHTTPRequestHandler):
    ...
    def handle_work_location(self):
        ...
        Served from data/worklocation/YYYY-MM.json. A missing or stale month is refreshed from
        GreytHR (read-only) in the background; the reply says "building" or "refreshing" meanwhile."""
        import re
        from urllib.parse import parse_qs
        ...
        try:
            ...
            payload = work_location_store.get_month(month)
        except Exception as exc:
            print(f"[work-location] {month} failed: {type(exc).__name__}: {exc}", flush=True)
            self.send_json(
                {"error": "Work location data could not be loaded for that month. Please try again in a moment."},
                HTTPStatus.BAD_GATEWAY,
            )
            return
        self.send_json(payload)
